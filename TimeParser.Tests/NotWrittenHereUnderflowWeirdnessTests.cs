...
namespace TimeSpanParserUtil.Tests {
    ...
    public class NotWrittenHereUnderflowWeirdnessTests {
        ...
        public void IfThisTestFailsThenDotNetBugsHaveBeenFixed() { // aka TimeSpanWeirdnessDemo
            ...
            Assert.AreEqual(TimeSpan.Parse("0:00:00.00000098").Ticks, 98); // Still passes and still shouldn't
            Assert.ThrowsException<OverflowException>(() => 
                            TimeSpan.Parse("0:00:00.00000099")); // Uniquely, but correctly (perhaps), causes an overflow

            Assert.AreEqual(TimeSpan.Parse("0:00:00.0000000"), TimeSpan.Zero); // Passes correctly (7 fractional zeroes)
            Assert.ThrowsException<OverflowException>(() => 
                            TimeSpan.Parse("0:00:00.00000000")); // Needlessly overflows (8 fractional zeroes)
